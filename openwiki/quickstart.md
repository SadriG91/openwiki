...
- Supports multiple model providers — OpenRouter (default), Anthropic, OpenAI, Baseten, Fireworks, and an OpenAI-compatible option — each with their own API key and model list (the OpenAI-compatible provider has no preset model list and requires a user-supplied base URL).
